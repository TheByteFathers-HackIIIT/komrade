...
export default async function newOrder(req: Request,res: Response){
    ...
    let orderobj = new OrderModel({ordererNumber:orderer.phone, ordererEmail: orderer.email, items: order.items, expire: 0, canteen: order.canteen, timestamp: Date.now()/1000, completed: false});
    await orderobj.save();
    res.send("Success!");
}
